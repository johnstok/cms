...
public class AlphaNumericAscendingNameComparatorTest
    ...
    public void testCaseSensitiveSort() {
        ...
        // ASSERT
        assertEquals("aaa", unsorted.get(0).name().toString());
        assertEquals("mmm", unsorted.get(1).name().toString());
        assertEquals("zzz", unsorted.get(2).name().toString());
    }
    ...
    public void testCaseInsensitiveSort() {
        ...
        // ASSERT
        assertEquals("aaa", unsorted.get(0).name().toString());
        assertEquals("AAA", unsorted.get(1).name().toString());
        assertEquals("zzz", unsorted.get(2).name().toString());
        assertEquals("ZZZ", unsorted.get(3).name().toString());
    }
}
