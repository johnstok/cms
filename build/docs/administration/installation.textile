...
* Copy the @topics-service.xml@ from the distribution to @<cc_dir>@
* Unzip the ear file from the distribution into @<cc_dir>/cc7.ear@
...
* Edit the @<cc_dir>/cc7.ear/log4j.properties@ file to specify an appropriate value for the @log4j.appender.CC.File@ setting.
...
* Create a symbolic link from the Jboss server deploy directory to the JMS configuration: @<cc_dir>/topics-service.xml@.
